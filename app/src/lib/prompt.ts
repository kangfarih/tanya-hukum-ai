...
/**
 * Loads the versioned system prompt (prompts/system-v1.md) — committed, not inline.
 *
 * Candidates, in order:
 *   1. <repo>/prompts/system-v1.md            — monorepo local dev (app/ is a subfolder)
 *   2. app/src/prompts/system-v1.md           — deployed copy when Vercel root = app/
 *
...
export async function readSystemPrompt(): Promise<string> {
  const candidates = [
    ...
    path.resolve(process.cwd(), "src/prompts/system-v1.md"),
  ];
  ...
  throw new Error("system prompt not found — expected prompts/system-v1.md");
}
